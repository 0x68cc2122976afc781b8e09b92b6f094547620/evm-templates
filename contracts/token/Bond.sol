...
/**
 * @title XFT Tokenized Fixed Income Protocol
    ...
    EIP712Upgradeable
{
    ...
    // Bond Errors
    error BondInvalidMintReceiver(address receiver);
    error BondInvalidBurnSender(address sender);
    error BondInsufficientBurnBalance(address sender, uint256 shares, uint256 sharesNeeded);
    error BondInvalidRewardMultiplier(uint256 rewardMultiplier);
    error BondBlockedSender(address sender);
    error BondInvalidBlockedAccount(address account);
    error BondPausedTransfers();
     ...
     */
    function convertToBonds(uint256 amount) public view returns (uint256) {
    ...
    function totalSupply() external view returns (uint256) {
        return convertToTokens(_totalBonds);
    }
    ...
    function _mint(address to, uint256 amount) private {
        if (to == address(0)) {
            revert BondInvalidMintReceiver(to);
        }

        _beforeTokenTransfer(address(0), to, amount);

        uint256 shares = convertToBonds(amount);
        _totalBonds += shares;

        unchecked {
            // Overflow not possible: shares + shares amount is at most totalBonds + shares amount
    ...
    function _burn(address account, uint256 amount) private {
        if (account == address(0)) {
            revert BondInvalidBurnSender(account);
        }

        _beforeTokenTransfer(account, address(0), amount);

        uint256 shares = convertToBonds(amount);
        uint256 accountBonds = sharesOf(account);

        if (accountBonds < shares) {
            revert BondInsufficientBurnBalance(account, accountBonds, shares);
        }

        unchecked {
            _shares[account] = accountBonds - shares;
            // Overflow not possible: amount <= accountBonds <= totalBonds.
            _totalBonds -= shares;
        }
    ...
    function _beforeTokenTransfer(address from, address /* to */, uint256 /* amount */) private view {
        ...
        if (isBlocked(from)) {
            revert BondBlockedSender(from);
        }
        // Useful for scenarios such as preventing trades until the end of an evaluation
        // period, or having an emergency switch for freezing all token transfers in the
        // event of a large bug.
        if (paused()) {
            revert BondPausedTransfers();
        }
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        uint256 shares = convertToBonds(amount);
        uint256 fromBonds = _shares[from];

        if (fromBonds < shares) {
            revert ERC20InsufficientBalance(from, fromBonds, shares);
        }

        unchecked {
            _shares[from] = fromBonds - shares;
            // Overflow not possible: the sum of all shares is capped by totalBonds, and the sum is preserved by
    ...
    function _blockAccount(address account) private {
        if (isBlocked(account)) {
            revert BondInvalidBlockedAccount(account);
        }
    ...
    function _setRewardMultiplier(uint256 _rewardMultiplier) private {
        if (_rewardMultiplier < _BASE) {
            revert BondInvalidRewardMultiplier(_rewardMultiplier);
        }
    ...
    function addRewardMultiplier(uint256 _rewardMultiplierIncrement) external onlyRole(ORACLE_ROLE) {
        if (_rewardMultiplierIncrement == 0) {
            revert BondInvalidRewardMultiplier(_rewardMultiplierIncrement);
        }
