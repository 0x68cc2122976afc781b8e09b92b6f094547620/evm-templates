...
/**
 * @title XFT Central Bank Digital Currency Protocol
    ...
    EIP712Upgradeable
{
    ...
    // CBDC Errors
    error CBDCInvalidMintReceiver(address receiver);
    error CBDCInvalidBurnSender(address sender);
    error CBDCInsufficientBurnBalance(address sender, uint256 shares, uint256 sharesNeeded);
    error CBDCInvalidRewardMultiplier(uint256 rewardMultiplier);
    error CBDCBlockedSender(address sender);
    error CBDCInvalidBlockedAccount(address account);
    error CBDCPausedTransfers();
     ...
     */
    function convertToCBDCs(uint256 amount) public view returns (uint256) {
     ...
     */
    function totalCBDCs() external view returns (uint256) {
        return _totalCBDCs;
    }
    ...
    function totalSupply() external view returns (uint256) {
        return convertToTokens(_totalCBDCs);
    }
    ...
    function _mint(address to, uint256 amount) private {
        if (to == address(0)) {
            revert CBDCInvalidMintReceiver(to);
        }

        _beforeTokenTransfer(address(0), to, amount);

        uint256 shares = convertToCBDCs(amount);
        _totalCBDCs += shares;

        unchecked {
            // Overflow not possible: shares + shares amount is at most totalCBDCs + shares amount
    ...
    function _burn(address account, uint256 amount) private {
        if (account == address(0)) {
            revert CBDCInvalidBurnSender(account);
        }

        _beforeTokenTransfer(account, address(0), amount);

        uint256 shares = convertToCBDCs(amount);
        uint256 accountCBDCs = sharesOf(account);

        if (accountCBDCs < shares) {
            revert CBDCInsufficientBurnBalance(account, accountCBDCs, shares);
        }

        unchecked {
            _shares[account] = accountCBDCs - shares;
            // Overflow not possible: amount <= accountCBDCs <= totalCBDCs.
            _totalCBDCs -= shares;
        }
    ...
    function _beforeTokenTransfer(address from, address /* to */, uint256 /* amount */) private view {
        ...
        if (isBlocked(from)) {
            revert CBDCBlockedSender(from);
        }
        // Useful for scenarios such as preventing trades until the end of an evaluation
        // period, or having an emergency switch for freezing all token transfers in the
        // event of a large bug.
        if (paused()) {
            revert CBDCPausedTransfers();
        }
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        uint256 shares = convertToCBDCs(amount);
        uint256 fromCBDCs = _shares[from];

        if (fromCBDCs < shares) {
            revert ERC20InsufficientBalance(from, fromCBDCs, shares);
        }

        unchecked {
            _shares[from] = fromCBDCs - shares;
            // Overflow not possible: the sum of all shares is capped by totalCBDCs, and the sum is preserved by
    ...
    function _blockAccount(address account) private {
        if (isBlocked(account)) {
            revert CBDCInvalidBlockedAccount(account);
        }
    ...
    function _setRewardMultiplier(uint256 _rewardMultiplier) private {
        if (_rewardMultiplier < _BASE) {
            revert CBDCInvalidRewardMultiplier(_rewardMultiplier);
        }
    ...
    function addRewardMultiplier(uint256 _rewardMultiplierIncrement) external onlyRole(ORACLE_ROLE) {
        if (_rewardMultiplierIncrement == 0) {
            revert CBDCInvalidRewardMultiplier(_rewardMultiplierIncrement);
        }
